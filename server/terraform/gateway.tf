...
resource "aws_api_gateway_method" "proxy_root" {
  rest_api_id   = "${aws_api_gateway_rest_api.logging_API.id}"
  resource_id   = "${aws_api_gateway_rest_api.logging_API.root_resource_id}"
  http_method   = "ANY"
  authorization = "NONE"
}
...
resource "aws_api_gateway_integration" "lambda" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "${aws_lambda_function.orientation_logger_lambda.invoke_arn}"
}
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = "${aws_lambda_function.orientation_logger_lambda.arn}"
